...
export async function readDoctorMaintenanceRecoveryConfig(
  ...
  return resources.run(async () => {
    const { config, valid } = await readConfigFileSnapshot({
      skipPluginValidation: true,
      observe: false,
    });
    // Fail closed: a malformed or otherwise invalid saved config must never be
    // treated as an empty config and handed to Gateway restoration. The
    // snapshot maps a JSON5 parse failure to `valid: false` with `config: {}`,
    // so returning `config` unchecked would let restoration proceed on an
    // empty config. Refusing here keeps the Gateway stopped instead.
    if (!valid) {
      throw new DoctorMaintenanceRefusalError(
        "Doctor cannot restore the Gateway: the saved recovery config is invalid, so the Gateway stays stopped.",
